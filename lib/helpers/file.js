...
export const isTypeOfImage = file => {
  return TypesFiles.imgTypes.map(type => {
    if (file.type === type) {
      return true;
    }
    return false;
...
// Check for Image Type
export const isTypeOfInvalid = file => {
  return TypesFiles.invalidTypes.map(type => {
    if (file.type === type) {
      return true;
    }
    return false;
...
// Check for PDF Type
export const isTypeOfPDF = file => {
  return TypesFiles.pdfTypes.map(type => {
    if (file.type === type) {
      return true;
    }
    return false;
...
export const isTypeOfFilename = file => {
  return TypesStrings.stringTypes.map(type => {
    if (file.fileName.substr(file.fileName.length - 3) === type) {
      return true;
    }
    return false;
  });
...
export const isStringOfImgType = file => {
  return TypesStrings.stringTypes.map(type => {
    if (file.fileName.substr(file.fileName.length - 3) === type) {
      return true;
    }
...
export const isStringOfPDF = file => {
  return TypesStrings.pdfStringTypes.map(type => {
    if (file.fileName.substr(file.fileName.length - 3) === type) {
      return true;
    }
    return false;
...
export const isFileUploadSize = file => {
  return file.size <= TypesEditor.MAX_FILE_UPLOAD_SIZE;
};
...
export function makeUniqueFileName(name, uniqueKey) {
  return `${getFileName(name)}-${uniqueKey}.${getFileExtension(name)}`;
}
...
export function evalUniqueName(name, uniqueKey) {
  if (isNameUnique(name)) {
    return name;
  }
  return sanitizeFileName(makeUniqueFileName(name, uniqueKey));
}
