...
import ArrowDropDownIcon from '@symbols/core/internal/svg-icons/ArrowDropDown';
...
class SelectInput extends React.Component {
  ...
  render() {
    ...
    if (native) {
      warning(
        multiple === false,
        'Storyline-UI: you can not use the `native` and `multiple` properties ' +
          'at the same time on a `Select` component.'
      );
      warning(
        !renderValue,
        'Storyline-UI: the `renderValue` property is not used by the native implementation.'
      );
      warning(
        !displayEmpty,
        'Storyline-UI: the `displayEmpty` property is not used by the native implementation.'
      );
    ...
    if (value === undefined) {
      throw new Error(
        'Storyline-UI: the `value` property is required ' +
    ...
    const items = React.Children.map(children, child => {
      ...
      if (multiple) {
        if (!Array.isArray(value)) {
          throw new Error(
            'Storyline-UI: the `value` property must be an array ' +
