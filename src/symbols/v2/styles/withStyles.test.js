...
describe('withStyles', () => {
  ...
  describe('props', () => {
    ...
    before(() => {
      ...
      StyledComponent1 = withStyles(styles, { name: 'SSTextField' })(Empty);
  ...
  describe('mount', () => {
    ...
    it('should run lifecycles with no theme', () => {
      ...
      const wrapper = mount(
        <SSThemeProvider theme={createSSTheme()}>
      ...
      assert.deepEqual(sheetsRegistry.registry[0].classes, { root: 'Empty-root-1' });
      wrapper.setProps({ theme: createSSTheme() });
      assert.strictEqual(sheetsRegistry.registry.length, 1, 'should only attach once');
      assert.deepEqual(sheetsRegistry.registry[0].classes, { root: 'Empty-root-1' });
    ...
    it('should support theme.props', () => {
      const styles = { root: { display: 'flex' } };
      const StyledComponent = withStyles(styles, { name: 'SSFoo' })(Empty);
    ...
    it('should work when depending on a theme', () => {
      ...
      const StyledComponent = withStyles(styles, { name: 'SSTextField' })(Empty);
      ...
      assert.strictEqual(sheetsRegistry.registry.length, 1, 'should only attach once');
      assert.deepEqual(sheetsRegistry.registry[0].classes, { root: 'SSTextField-root-1' });
      wrapper.setProps({ theme: createSSTheme() });
      assert.strictEqual(sheetsRegistry.registry.length, 1, 'should only attach once');
      assert.deepEqual(sheetsRegistry.registry[0].classes, { root: 'SSTextField-root-2' });
    });
    ...
    describe('options: disableStylesGeneration', () => {
      it('should not generate the styles', () => {
        ...
        const wrapper = mount(
          <SSThemeProvider theme={createSSTheme()} disableStylesGeneration>
            <JssProvider registry={sheetsRegistry} jss={jss} generateClassName={generateClassName}>
              <StyledComponent />
            </JssProvider>
          </SSThemeProvider>,
  ...
  describe('HMR with same state', () => {
    it('should take the new stylesCreator into account', () => {
      ...
      const StyledComponent1 = withStyles(styles1, { name: 'SSTextField' })(Empty);
      const wrapper = mount(<StyledComponent1 />);

      const styles2 = { root: { padding: 2 } };
      const StyledComponent2 = withStyles(styles2, { name: 'SSTextField' })(Empty);
