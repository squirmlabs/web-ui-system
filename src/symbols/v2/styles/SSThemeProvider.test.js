...
import SSThemeProvider from './SSThemeProvider';
...
function getOptionsSpy() {
  ...
  const OptionsSpy = (props, context) => {
    optionsSpy(context.ssThemeProviderOptions);
    return props.children;
  };
...
describe('<SSThemeProvider />', () => {
  ...
  describe('server side', () => {
    ...
    function assertRendering(markup, sheetsRegistry) {
      ...
      assert.strictEqual(sheetsRegistry.registry[0].classes.root, 'SSButtonBase-root-27');
      assert.deepEqual(
        sheetsRegistry.registry[0].classes,
        {
          disabled: 'SSButtonBase-disabled-28',
          focusVisible: 'SSButtonBase-focusVisible-29',
          root: 'SSButtonBase-root-27',
        },
        'the class names should be deterministic',
      );
      assert.strictEqual(sheetsRegistry.registry[1].classes.root, 'SSButton-root-1');
    }
    ...
    it('should be able to extract the styles', () => {
      ...
      const markup = ReactDOMServer.renderToString(
        <JssProvider registry={sheetsRegistry} generateClassName={generateClassName}>
          <SSThemeProvider theme={theme} sheetsManager={new Map()}>
    ...
    it('should be able to cache the sheets between two requests', () => {
      ...
      const markup1 = ReactDOMServer.renderToString(
        <JssProvider registry={sheetsRegistry1} generateClassName={generateClassName}>
          <SSThemeProvider theme={theme} sheetsManager={new Map()} sheetsCache={sheetsCache}>
            <Button>Hello World</Button>
          </SSThemeProvider>
        </JssProvider>,
      );
  ...
  describe('mount', () => {
    it('should work with nesting theme', () => {
      ...
      const theme1 = createSSTheme({ status: { color: 'orange' } });
      const theme2 = outerTheme => ({ ...outerTheme, status: { color: 'green' } });
      const theme3 = createSSTheme({ status: { color: 'yellow' } });

      const wrapper = mount(
        <SSThemeProvider theme={theme1}>
          <ThemeSpy1>
            <SSThemeProvider theme={theme2}>
              <ThemeSpy2>
                <span>Foo</span>
              </ThemeSpy2>
            </SSThemeProvider>
            <SSThemeProvider theme={theme3}>
              <ThemeSpy3>
                <span>Bar</span>
              </ThemeSpy3>
            </SSThemeProvider>
      ...
      wrapper.setProps({ theme: createSSTheme({ status: { color: 'blue' } }) });
    ...
    it('should forward the parent options', () => {
      const theme = createSSTheme({ status: { color: 'orange' } });
      ...
      OptionsSpy.contextTypes = {
        ssThemeProviderOptions: PropTypes.object.isRequired,
      };

      mount(
        <SSThemeProvider theme={theme} disableStylesGeneration>
  ...
  describe('prop: sheetsCache', () => {
    it('should provide the property down the context', () => {
      ...
      const sheetsCache = new Map();
      mount(
        <SSThemeProvider theme={theme} sheetsCache={sheetsCache}>
          <OptionsSpy>
            <div>Foo</div>
          </OptionsSpy>
        </SSThemeProvider>,
