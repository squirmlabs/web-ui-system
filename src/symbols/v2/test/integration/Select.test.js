  ...
  describe('with Dialog', () => {
    it('should focus the selected item', done => {
      ...
      const portalLayer = wrapper
        ...
        .instance()
        .getMountNode();
      const selectDisplay = portalLayer.querySelector('[data-ss-test="SelectDisplay"]');
      ...
      const dialogPortalLayer = document.querySelectorAll('[data-ss-test="Modal"]')[1];
      ...
      const backdrop = dialogPortalLayer.querySelector('[data-ss-test="Backdrop"]');
      if (backdrop) {
        backdrop.click();
      }
    });
