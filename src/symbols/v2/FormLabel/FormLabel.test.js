...
describe('<FormLabel />', () => {
  ...
  describe('prop: required', () => {
    it('should show an asterisk if required is set', () => {
      ...
      assert.strictEqual(wrapper.find('[data-ss-test="FormLabelAsterisk"]').length, 1);
    });

    it('should not show an asterisk by default', () => {
      const wrapper = shallow(<FormLabel />);
      assert.strictEqual(wrapper.find('[data-ss-test="FormLabelAsterisk"]').length, 0);
  ...
  describe('prop: error', () => {
    it('should have an error class', () => {
      ...
      const asteriskWrapper = wrapper.find('[data-ss-test="FormLabelAsterisk"]');
  ...
  describe('with ssFormControl context', () => {
    ...
    function setFormControlContext(ssFormControlContext) {
      ...
      wrapper.setContext({ ssFormControl });
    }
