...
export function capitalize(string) {
  if (process.env.NODE_ENV !== 'production' && typeof string !== 'string') {
    throw new Error('Storyline-UI: capitalize(string) expects a string argument.');
  }
  ...
  return funcs.filter(func => func != null).reduce(
    (acc, func) => {
      warning(
        ...
        'Storyline-UI: invalid Argument Type, must only provide functions, undefined, or null.',
      );
