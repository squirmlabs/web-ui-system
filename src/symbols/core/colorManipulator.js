...
/**
 * Returns anumber whose value is limited to the given range.
 *
...
function clamp(value, min = 0, max = 1) {
  warning(
    value >= min && value <= max,
    `Storyline-UI: the value provided ${value} is out of range [${min}, ${max}].`
  );
 ...
 * @returns {object} - A UI color object: {type: string, values: number[]}
 */
...
export function getLuminance(color: string) {
  ...
  throw new Error(`Storyline-UI: unsupported \`${color}\` color.`);
}
...
export function fade(color: string, value: number) {
  warning(color, `Storyline-UI: missing color argument in fade(${color}, ${value}).`);
...
export function darken(color: string, coefficient: number) {
  warning(color, `Storyline-UI: missing color argument in darken(${color}, ${coefficient}).`);
...
export function lighten(color: string, coefficient: number) {
  warning(color, `Storyline-UI: missing color argument in lighten(${color}, ${coefficient}).`);
