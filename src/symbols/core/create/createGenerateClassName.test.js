...
import consoleErrorMock from '../../lib/test/utils/consoleErrorMock';
  ...
  it('should escape parenthesis', () => {
    ...
    assert.strictEqual(
      generateClassName(
        ...
        {
          options: {
            classNamePrefix: 'pure(UIButton)',
            jss: {}
          }
        }
      ),
      'pure-UIButton--root-1'
    );
  });
  ...
  describe('formatting', () => {
    ...
    describe('production', () => {
      ...
      it('should output a short representation', () => {
        const rule = { key: 'root' };
        const generateClassName = createGenerateClassName();
        assert.strictEqual(generateClassName(rule), 'SquirmlabsComponent1');
      });
      ...
      it('should warn', () => {
        ...
        assert.match(
          ...
          /Storyline-UI: we have detected more than needed creation of the/
        );
      });
    });
  });
