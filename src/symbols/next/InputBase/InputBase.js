...
export function formControlState({ props, states, context }) {
  return states.reduce((acc, state) => {
    ...
    if (context && context.ssFormControl) {
      if (typeof props[state] === 'undefined') {
        acc[state] = context.ssFormControl[state];
      }
    }
...
class InputBase extends React.Component {
  constructor(props, context) {
    ...
    const componentWillUpdate = (nextProps, nextState, nextContext) => {
      ...
      if (
        ...
        const { ssFormControl } = this.context;
        if (ssFormControl && ssFormControl.onBlur) {
          ssFormControl.onBlur();
        }
      }
    };
  ...
  getChildContext() {
    // We are consuming the parent ssFormControl context.
  ...
  handleFocus = event => {
    ...
    const { ssFormControl } = this.context;
    if (ssFormControl && ssFormControl.onFocus) {
      ssFormControl.onFocus(event);
    }
  };
  ...
  checkDirty(obj) {
    ...
    if (isFilled(obj)) {
      if (ssFormControl && ssFormControl.onFilled) {
        ssFormControl.onFilled();
      }
    ...
    if (ssFormControl && ssFormControl.onEmpty) {
      ssFormControl.onEmpty();
    }
  ...
  render() {
    ...
    const className = classNames(
      classes.root,
      {
        ...
        [classes.formControl]: ssFormControl,
...
InputBase.contextTypes = {
  ssFormControl: PropTypes.object,
};
...
export default withStyles(styles, { name: 'SSInputBase' })(InputBase);
