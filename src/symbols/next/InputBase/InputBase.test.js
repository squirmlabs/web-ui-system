...
describe('<InputBase />', () => {
  ...
  describe('with ssFormControl context', () => {
    let wrapper;
    let ssFormControl;

    function setFormControlContext(ssFormControlContext) {
      ssFormControl = ssFormControlContext;
      wrapper.setContext({ ...wrapper.context(), ssFormControl });
    }
    ...
    describe('callbacks', () => {
      ...
      it('should fire the onFilled ssFormControl and props callback when dirtied', () => {
        ...
        wrapper.find('input').simulate('change');
        assert.strictEqual(handleFilled.callCount, 1);
        assert.strictEqual(ssFormControl.onFilled.callCount, 1);
      });

      it('should fire the onEmpty ssFormControl and props callback when cleaned', () => {
        ...
        wrapper.find('input').simulate('change');
        assert.strictEqual(handleEmpty.callCount, 1);
        assert.strictEqual(ssFormControl.onEmpty.callCount, 1);
      });

      it('should fire the onFocus ssFormControl', () => {
        ...
        assert.strictEqual(ssFormControl.onFocus.callCount, 1);
      });

      it('should fire the onBlur ssFormControl', () => {
        const handleBlur = spy();
        wrapper.setProps({
          onBlur: handleBlur,
        });
        ...
        assert.strictEqual(handleBlur.callCount, 1);
        assert.strictEqual(ssFormControl.onBlur.callCount, 1);
      });
